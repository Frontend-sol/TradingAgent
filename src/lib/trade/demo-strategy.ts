...
function buildAutoExpandedMarketAppendix(context: TemplateContext, symbols: string[]) {
  const instIds = [...new Set(symbols.map((item) => normalizeInstId(item)).filter(Boolean))];
  const rows = instIds.map((instId) => {
    const coin = instId.split("-")[0] || instId;
    return buildCoinAuditBlock(context, coin);
  });

  return [
    "",
    "## AUTO-EXPANDED SELECTED SYMBOL MARKET DATA",
    "",
    "The following JSON is generated by the Node server from the selected trading symbols. It follows the same data template as BTC/ETH placeholders, so new symbols do not require manual prompt placeholders.",
    "Use this block only as market context. The executable decision for this run must target the current `symbol` at the top of the prompt.",
    "```json",
...
function buildFinalPrompt(basePrompt: string, context: TemplateContext, selectedSymbols: string[]) {
  return [
    basePrompt,
    buildAccountPromptAppendix(context),
    buildAutoExpandedMarketAppendix(context, selectedSymbols),
    "",
    "Based on the above data, provide your trading decision in the required JSON format.",
  ].join("\n");
}

function getSelectedAliasCoins(symbols: string[]) {
  return symbols
    .map((item) => normalizeInstId(item).split("-")[0]?.toLowerCase())
...
async function resolveTemplateContext(input: ResolveContextInput): Promise<ResolvedTemplateContext> {
  ...
  const selectedAliasCoins = getSelectedAliasCoins(input.symbols);
  const autoExpandedAliasCoins = [...new Set([...requestedAliasCoins, ...selectedAliasCoins])];
  ...
  const shouldAutoExpandSelectedSymbols = autoExpandedAliasCoins.length > 0;
  const aliasNeedsMarket = shouldAutoExpandSelectedSymbols || [
    "price",
    "ema20",
    "ema50",
    "macd",
  ...
  const aliasNeedsIndicators = shouldAutoExpandSelectedSymbols || ["ema20", "ema50", "macd", "rsi7", "rsi14", "ema20_3m", "macd_3m", "rsi7_3m", "rsi14_3m"].some(
    (metric) => aliasMetricSet.has(metric),
  );
  const aliasNeeds4h = shouldAutoExpandSelectedSymbols || ["ema20_4h", "ema50_4h", "atr3_4h", "atr14_4h", "volume_current_4h", "volume_avg_4h", "volume_current", "volume_avg", "macd_4h", "rsi14_4h"].some(
    (metric) => aliasMetricSet.has(metric),
  );
  const aliasNeedsFunding = shouldAutoExpandSelectedSymbols || aliasMetricSet.has("funding_rate");
  const aliasNeedsOpenInterest = shouldAutoExpandSelectedSymbols || aliasMetricSet.has("oi_latest") || aliasMetricSet.has("oi_avg");
  ...
  const context: TemplateContext = {
    ...
    selected_symbols: input.symbols.map((item) => normalizeInstId(item)),
  ...
  for (const aliasCoin of autoExpandedAliasCoins) {
...
function buildPromptAuditLog(context: TemplateContext, requestedVariables: string[], renderedPrompt: string) {
  const requestedCoins = [...new Set([
    ...requestedVariables.map((item) => getDynamicAliasCoin(item)).filter((item): item is string => Boolean(item)),
    ...getSelectedAliasCoins(Array.isArray(context.selected_symbols) ? context.selected_symbols.map(String) : []),
  ])];
  return {
...
export async function runStrategy(symbol: string, strategyId?: string) {
  ...
  try {
    resolvedContext = await resolveTemplateContext({
      template,
      instId,
      symbols: strategy.symbols.length ? strategy.symbols : [instId],
  ...
  const selectedSymbols = strategy.symbols.length ? strategy.symbols : [instId];
  const prompt = buildFinalPrompt(basePrompt, resolvedContext.context, selectedSymbols);
